...
CREATE TABLE IF NOT EXISTS edges (
    id VARCHAR(36) PRIMARY KEY,
    graph_id VARCHAR(36) NOT NULL REFERENCES graphs(id) ON DELETE CASCADE,
    from_id VARCHAR(36) NOT NULL REFERENCES nodes(id) ON DELETE CASCADE,
    to_id VARCHAR(36) NOT NULL REFERENCES nodes(id) ON DELETE CASCADE,
    label VARCHAR(255),
    weight FLOAT DEFAULT 1.0,
    metadata JSONB,
    created_at TIMESTAMP NOT NULL DEFAULT NOW(),
    CONSTRAINT different_nodes CHECK (from_id != to_id)
);
